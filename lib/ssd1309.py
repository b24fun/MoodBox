...
class Display(object):
    ...
    def draw_arc(self, x0, y0, r, start_angle, end_angle, invert=False):
        """Draw a perfectly symmetrical circular arc."""
        # Ensure angles are in increasing order
        if start_angle > end_angle:
            start_angle, end_angle = end_angle, start_angle

        # Use fine step size for smoother, more symmetrical arcs
        steps = int(r * (end_angle - start_angle))  # proportional to radius and arc length
        for i in range(steps + 1):
            angle = radians(start_angle + (end_angle - start_angle) * i / steps)
            x = int(round(x0 + r * cos(angle)))
            y = int(round(y0 + r * sin(angle)))
            self.draw_pixel(x, y, invert)
